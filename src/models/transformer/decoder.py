...
class TransformerDecoder(nn.Module):
    def __init__(self, num_layers, d_model, num_heads, dropout=0.0, future_mask=True):
        ...
        self.progressive_drop_index = None
        self.progressive_keep_ratio = 1.0
    ...
    def forward(self, src, src_mask, tgt, tgt_mask, src_gauss_weight=None, tgt_gauss_weight=None):
        non_pad_src_mask = None if src_mask is None else ~src_mask.bool()
        non_pad_tgt_mask = None if tgt_mask is None else ~tgt_mask.bool()
        ...
        for layer_idx, layer in enumerate(self.decoder_layers):
            residual = x
            updated, weight = layer(x, non_pad_tgt_mask,
                                    src, non_pad_src_mask,
                                    self.buffered_future_mask(x),
                                    src_gauss_weight, tgt_gauss_weight)
            if layer_idx == self.progressive_drop_index:
                x = residual + self.progressive_keep_ratio * (updated-residual)
            else:
                x = updated
            if self.capture_hidden:
                hidden_states.append(x.transpose(0, 1))
        self.last_hidden_states = hidden_states if self.capture_hidden else None
