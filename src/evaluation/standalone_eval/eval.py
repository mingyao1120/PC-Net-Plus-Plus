...
def compute_mr_ap(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10),
    ...
    if ap_array.shape[0] == 0:
        ap_array = np.zeros((1,10),dtype=np.float32)
        ap_thds = np.zeros(10)
    else: 
...
def compute_mr_r1(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10)):
    ...
    if len(submission) == 0:
        pred_windows = np.zeros((10,2), dtype=np.float32)
        gt_windows = np.zeros((10,2), dtype=np.float32)
    else: 
    ...
    # calculate mIoU
    iou_thd2recall_at_one["average"] = float(f"{np.sum(pred_gt_iou) / len(pred_gt_iou) * 100:.2f}")
...
def mk_gt_scores(gt_data, clip_length=2):
    ...
    for idx, id in enumerate(relevant_clip_ids):
        if id >= saliency_scores_full_video.shape[0]:
            break
        saliency_scores_full_video[id] = saliency_scores_relevant_clips[idx]
...
def eval_submission(submission, ground_truth, verbose=True, match_number=True):
    ...
    if "pred_relevant_windows" in submission[0]:
        ...
        moment_ret_scores_brief = {
            ...
            "MR-full-mIoU": moment_ret_scores["full"]["MR-R1"]["average"],  # add
        }
        eval_metrics_brief.update(
            sorted([(k, v) for k, v in moment_ret_scores_brief.items()], key=lambda x: x[0]))

    if any("pred_saliency_scores" in d for d in submission):
